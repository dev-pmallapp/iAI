...
| 6 | Each of the four skills carries the hard-failure block for its own required input, and the three that resolve a domain binding read the label before the step that needs it | CLAIM-41.5 | iai-skills | P0 | tool-checked | real — the four skill bodies | `bun test` | **4 of 4 carry the block; 3 of 3 order correctly.** Both counts asserted before the loop, and **the two populations are asserted to partition the corpus exactly** — named and counted, never derived, per the #307 ruling. A new verb unclassified in either list breaks the total |
...
| 12 | A skill whose own required input is absent produces the hard-failure block, not an assumed default | CLAIM-41.5, CLAIM-41.1 | iai-skills | P0 | tool-checked | synthetic — a fixture with the required input removed | `bun test` | Each of the four emits the block for **its own** trigger — a missing `domain:*` label for the three that resolve one, an unresolvable goal for the one that does not. **Asserting the absence of a default is the case**: a skill that silently picked a domain would pass any test that only checked it did not crash |
...
| 4 | Case 23 is **strengthened**: the doc/linter agreement is asserted as **set equality in BOTH directions** over a complete rule-id table, not as one-directional resolution of the capabilities the prose happens to name | The case as written asks that "every capability the document claims resolves to a rule id that exists". **A one-directional check cannot see the defect at HEAD.** `CONTRIBUTING.md:128-129` names two capabilities that are not rules — but any mechanism keyed on *marked* capabilities passes an **unmarked** false sentence, and the next contributor reproduces `:128-129` verbatim. Set equality over a complete table makes the claimed set complete *by construction*: a capability that is not a rule has nowhere to live except a row that fails membership. `CLAIM-41.10` says the document and the linter **agree**, which is symmetric, so the reverse direction is the claim's other half rather than an extension of it — and it **immediately found** that all four body rules were undocumented, including `domain-routing-form`, shipped by #46 three commits earlier on this same branch. Precedent: `scripts/lint.ts:580-589`'s `RULE_SCOPES` and `packages/core/src/guards/path-allowlist.ts:16-20`, "THE COUNT ABOVE IS TESTED, NOT PROSE" |
| 5 | Case 24's tool moves from `bun run claim-lint` to **`bun test`**, and `claim-lint` gains a **per-scope denominator** | The case asks whether a **constant in the shim** holds a value, or whether a **comment** exists. Neither is a property `claim-lint` computes, and it has no rule that reads its own `SCOPE_DIRS`. **THIRD OCCURRENCE OF THE CATEGORY ERROR recorded as amendments 1 and 3, in three consecutive tasks** — and the sharpest instance yet, because unlike the first two it *looks* correctly pointed: `scripts/claim-lint.ts` genuinely **is** inside claim-lint's scanned population, since it lives under `scripts/`. **Being scanned is not being the subject of an assertion.** Separately, the case's `skills/` half was **unobservable**: `claim-lint` printed one conflated `N files scanned`, so `skills/` in scope and empty was indistinguishable from `skills/` not in scope. Adding `skills/` moved that total by exactly 1 (the `.gitkeep`) inside a total dominated by `docs/`. The per-scope denominator is the same medicine `testplan-corpus` (#289) and `design-spine` (#294) already take, for the same disease — and mutation N2, which left `skills` in `SCOPE_DIRS` while never walking it, is killed **only** by the run-time disk-equality assertion the denominator makes possible. `bun run claim-lint` remains the standing check that the widened scope yields 0 violations |
| 6 | **Recorded, not applied to the case rows:** #297 invalidated the *meaning* of every citation to `references/design-format.md:31`, including cases 17's and 2's own prose, **while being exactly line-neutral** | The edit was written to preserve line numbers — `:31`, `:52` and `:145` still exist and still hold the statements they held roles for, and both files changed 1:1 and 14:14 lines. **Line-neutrality preserved every citation's target and destroyed several citations' truth.** `:31` said *"Four keys: `phase`, `progress`, `task`, `slug`"* and now says there is no frontmatter at all, so `docs/design/stories/41.md:35`, `docs/test-plans/41-plan.md:140` (case 17) and **`docs/evidence/294-*.md:57` — which is immutable** — all still resolve to a live line that no longer says what they assert it says. The case rows are **not rewritten**, per the posture of amendments 1-5: they are the record of what was planned. **This is the sharpest evidence yet for #304**: a `:NN` guard that only checks the line still exists would have passed this edit unchanged, because making it pass was the entire design goal. The guard has to check what the line *says* |
