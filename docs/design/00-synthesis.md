...
|--------|----------------------|---------------------|
| **LifeOS** | **Intent** — TELOS, Current→Ideal State, Design, Cortex memory | *Why am I doing this, and what does "done" look like?* |
...
| **Goal / `/goal`** — a persistent objective re-injected on `session.idle` | **KEEP, merged with TELOS** | The mechanism is right and the content source is wrong: instead of a free-text goal, we re-inject the active Story's TELOS ancestry and its Design phase. Implemented via `experimental.chat.system.transform` on opencode, and via `SessionStart` context on Claude Code. Enhancement, never load-bearing |
