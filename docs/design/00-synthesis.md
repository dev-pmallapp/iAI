...
|--------|----------------------|---------------------|
| **LifeOS** | **Intent** — Goals, Current→Ideal State, Design, Cortex memory | *Why am I doing this, and what does "done" look like?* |
...
|-----------|---------|-----------|
| **Goals** — mission, problems, goals, strategies, beliefs, IDs `M0`/`P0`/`G0`/`S0` | **KEEP** | This is the intent root of the whole system. A Story with no path back to a `G0` is unjustified work. `iai-core/intent/` parses Goals and `goal-create` maps a `G0` to a GitHub Milestone. The ID scheme is preserved verbatim so existing LifeOS Goals files load unchanged |
...
| **Design v2.21.0** (LifeOS calls this the ISA) — YAML frontmatter (`phase`, `progress`, `task`, `slug`), 17 fixed body sections, claims as `- [ ] CLAIM-{story}.{n}:` checkboxes with `(after: ID)` dependencies, Test Strategy table with `anchors_to` / `severity` / `tier`, three verifier classes `tool-checked` / `model-judged` / `human-attested` | **KEEP** | This *is* our Story design doc. It is strictly better than forge's freeform `design-doc.md` because claims are addressable, dependency-ordered and machine-checkable. The three verifier classes are the reason Design generalises past code: `tool-checked` covers dev and wealth reconciliation, `model-judged` covers `know` distillation, `human-attested` covers health protocol adherence where no automated check exists. Stored at `docs/design/stories/{issue}.md` per the ARCHITECTURE data model |
...
| **Fabric** — 235 prompt patterns (309 `.md` files under `Patterns/`) | **DROP** | Out of scope. It is a prompt library, not a workflow system, and it duplicates what Tier-2 leaf skills do with worse traceability. Users who want Fabric can keep running Fabric |
...
| `## forge-design-doc` sentinel | `## iai-design` |
...
| **Goal / `/goal`** — a persistent objective re-injected on `session.idle` | **KEEP, merged with Goals** | The mechanism is right and the content source is wrong: instead of a free-text goal, we re-inject the active Story's Goals ancestry and its Design phase. Implemented via `experimental.chat.system.transform` on opencode, and via `SessionStart` context on Claude Code. Enhancement, never load-bearing |
...
- **The generalisation of Story and Task beyond code.** LifeOS files issues but
  ...
  `story-design` produce a Design for a *health protocol* or a *trading strategy*
...
| **Design doc format** | Design v2.20.0: YAML frontmatter, 17 fixed sections, `CLAIM-{story}.{n}` claims with `(after: ID)` deps, verifier classes | Freeform `design-doc.md` posted behind a `## forge-design-doc` sentinel | **Merge, Design-first.** iAI's Story design doc is the Design — frontmatter, 17 sections, claims, verifier classes — *plus* forge's `## Build Targets` table adopted as a section so the doc stays mechanically parseable. Published behind the renamed `## iai-design` sentinel with a SHA-pinned permalink. Lives at `docs/design/stories/{issue}.md` |
...
| **How much runs automatically** | Autonomic: 55 hooks acting continuously, daemons, background sync | Human gates at every consequential boundary; forge never merges | **Split by reversibility.** Reversible actions run automatically: ingestion, classification, checkpointing, status, screening, backtests, drafting. Irreversible actions are hard-gated behind a named authoriser: merging a PR, placing a live order, moving money, sending a clinician brief. The LifeOS `Daemon` is dropped precisely because it has no way to make that distinction |
