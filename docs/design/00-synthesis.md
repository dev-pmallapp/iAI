...
| **Design v2.21.0** (LifeOS calls this the ISA) — YAML frontmatter (`phase`, `progress`, `task`, `slug`), 17 fixed body sections, claims as `- [ ] ISC-N:` checkboxes with `(after: ID)` dependencies, Test Strategy table with `anchors_to` / `severity` / `tier`, three verifier classes `tool-checked` / `model-judged` / `human-attested` | **KEEP** | This *is* our Story design doc. It is strictly better than forge's freeform `design-doc.md` because ISC claims are addressable, dependency-ordered and machine-checkable. The three verifier classes are the reason Design generalises past code: `tool-checked` covers dev and wealth reconciliation, `model-judged` covers `know` distillation, `human-attested` covers health protocol adherence where no automated check exists. Stored at `docs/design/stories/{issue}.md` per the ARCHITECTURE data model |
...
| **Design doc format** | Design v2.20.0: YAML frontmatter, 17 fixed sections, `ISC-N` claims with `(after: ID)` deps, verifier classes | Freeform `design-doc.md` posted behind a `## forge-design-doc` sentinel | **Merge, Design-first.** iAI's Story design doc is the Design — frontmatter, 17 sections, ISC claims, verifier classes — *plus* forge's `## Build Targets` table adopted as a section so the doc stays mechanically parseable. Published behind the renamed `## iai-design` sentinel with a SHA-pinned permalink. Lives at `docs/design/stories/{issue}.md` |
