...
| **ISA / Ideal State Artifact v2.21.0** — YAML frontmatter (`phase`, `progress`, `task`, `slug`), 17 fixed body sections, claims as `- [ ] ISC-N:` checkboxes with `(after: ID)` dependencies, Test Strategy table with `anchors_to` / `severity` / `tier`, three verifier classes `tool-checked` / `model-judged` / `human-attested` | **KEEP** | This *is* our Story design doc. It is strictly better than forge's freeform `design-doc.md` because ISC claims are addressable, dependency-ordered and machine-checkable. The three verifier classes are the reason ISA generalises past code: `tool-checked` covers dev and wealth reconciliation, `model-judged` covers `know` distillation, `human-attested` covers health protocol adherence where no automated check exists. Stored at `docs/design/stories/{issue}.md` per the ARCHITECTURE data model |
