...
- [ ] CLAIM-194.1: No file under `docs/`, `scripts/`, `.github/` or the root
      markdown set contains the token `ISC-`, except for exactly three
      allow-listed paths: `docs/evidence/**` (immutable),
      `docs/design/stories/194.md` (this file, which *is* the mapping note) and
      `docs/test-plans/194-plan.md` (which must name the token in order to
      test for its absence). The allow-list is closed — a fourth path is a
      violation.
...
|-------|------|-------|-----------|------|------------|----------|
| CLAIM-194.1 | tool-checked | no `ISC-` token outside the three allow-listed paths | 0 hits | `git grep -n 'ISC-' -- docs scripts .github '*.md'`, minus `docs/evidence/**` and this file | literal | critical |
