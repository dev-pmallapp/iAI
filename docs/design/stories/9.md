---
task: "Bun workspace, 13 build targets, six CI checks"
...
workflow reports build, test, lint, typecheck, skill-lint and claim-lint as six
...
- The commit-msg guard enforces the regex adopted in this Design's Decisions
...
targets, and passes build, test, lint, typecheck, skill-lint and claim-lint
as six separately reported required PR checks; a malformed commit subject and a
...
- [ ] CLAIM-9.2: A pull request reports build, test, lint, typecheck,
      skill-lint and claim-lint as six separately named required checks;
      failing any one leaves the PR unmergeable (after: CLAIM-9.1).
...
- [ ] CLAIM-9.6: `docs/design/verification-pass.md` carries one row per
      assertion in `docs/design/`, each bearing exactly one of four verdicts
      against LifeOS, forge and oh-my-opencode: `confirmed`, `corrected`,
      `original` or `invented`. `invented` and `original` are separated by
      whether the assertion makes a source claim at all. An **`invented`** row
      states or implies that a source contains something no source does, and
      must name the commit that removed or corrected it. An **`original`** row
      is net-new iAI design that borrows no source's authority, so there is
      nothing to reconcile and no commit is required.
...
| CLAIM-9.2 | bash | branch protection requires exactly the six named check contexts | 6 contexts | `bash scripts/verify-required-checks.sh` | literal | critical |
...
| CLAIM-9.6 | manual | every design assertion carries exactly one of the four verdicts, and every `invented` row names its corrective commit | 100% verdicted, 0 blank, 24/24 commits | review of `docs/design/verification-pass.md` against the three source repos | literal | |
...
- 2026-08-25: Two conflicting commit-subject regexes were found in the repo.
  ...
  encoded inline). Issue #9's CLAIM-9.3 quotes the 03-workflow form, so this Design
...
- 2026-08-25: `{target_dir}` is defined twice, differently. `CONTRIBUTING.md:63`
  ...
  (example `src/telemetry`). This Design adopts the CONTRIBUTING form, since it
...
- 2026-08-25: `## Features` is deliberately omitted. LifeOS's Design spec makes
  ...
  This Design carries Claims and Build Targets, and no Features section.
